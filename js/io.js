...
function openScanImport(preferredSetId=''){
  ...
  renderScanReview();
  const preferredSet=(state.sets||[]).find(s=>s.id===preferredSetId),pendingTestCapture=!!preferredSet?.pendingTestPlan;
  if(pendingTestCapture&&$('#scanSetSelect'))$('#scanSetSelect').disabled=true;
...
function importScannedRows(){
  ...
  const targetSet=state.sets.find(x=>x.id===setId);if(targetSet){targetSet.pairReviewRequired=true;targetSet.pairVerifiedAt='';targetSet.pairVerifiedSignature='';}
...
function restore(text){
  try{
    const x=JSON.parse(text),issue=inspectBackup(x);if(issue)throw new Error(issue);const b=backupSummary(x),current=backupSummary(state),syncStatus=window.VTFamilySync?.status?.()||{enabled:false,role:'parent'};
    if(syncStatus.enabled&&syncStatus.role!=='parent'){
      modal('<div class="eyebrow">Backup einspielen</div><h2>Auf dem Kindergerät gesperrt</h2><p>Ein vollständiges Backup kann gemeinsame Familien- und Profildaten enthalten. Ein verbundenes Kindergerät darf diese Datenbereiche nicht überschreiben.</p><div class="modal-actions"><button value="cancel" class="primary">Verstanden</button></div>');
      return;
    }
    const backupIds=new Set((x.learners||[]).map(l=>String(l?.id||'')).filter(Boolean)),missingProfiles=syncStatus.enabled?(state.learners||[]).filter(l=>!backupIds.has(String(l.id||''))):[];
    if(missingProfiles.length){
      modal(`<div class="eyebrow">Backup einspielen</div><h2>Restore im Familiensync gestoppt</h2><p>Das Backup enthält nicht alle Profile des aktiven Familienverbunds.</p><div class="notice warn"><strong>Es wird nichts ersetzt.</strong><br>Die aktuelle Sync-Version kann das vollständige Löschen eines Profils noch nicht sicher an alle Geräte übertragen. Fehlend im Backup: ${missingProfiles.map(l=>esc(l.name||'Profil')).join(', ')}.</div><div class="modal-actions wrap"><button type="button" id="backupBeforeBlockedRestore" class="secondary">Aktuellen Stand sichern</button><button value="cancel" class="primary">Schließen</button></div>`);
      $('#backupBeforeBlockedRestore').onclick=backup;return;
    }
    modal(`<div class="eyebrow">Backup einspielen</div><h2>Aktuelle Daten ersetzen?</h2><p>Das Backup enthält <strong>${b.profiles} Profil${b.profiles===1?'':'e'}, ${b.sets} Lernsets und ${b.words} globale Vokabeln</strong>.</p><div class="notice warn">Aktuell auf diesem Gerät: ${current.profiles} Profil${current.profiles===1?'':'e'}, ${current.sets} Lernsets, ${current.words} globale Vokabeln. Diese Daten werden ersetzt.${syncStatus.enabled?' Der geprüfte Restore wird anschließend als neuer lokaler Familienstand zur Synchronisierung vorgemerkt.':''}</div><p class="microcopy">Empfehlung: Vorher ein aktuelles Backup herunterladen.</p><div class="modal-actions wrap"><button value="cancel" class="ghost">Abbrechen</button><button type="button" id="backupBeforeRestore" class="secondary">Vorher sichern</button><button type="button" id="confirmRestore" class="primary">Backup einspielen</button></div>`);
    $('#backupBeforeRestore').onclick=backup;
    $('#confirmRestore').onclick=async()=>{const btn=$('#confirmRestore');btn.disabled=true;btn.textContent='Wird gespeichert …';const previous=state;state=migrate(x);const ok=await persistState();let verified=false;if(ok){try{let check=null;if(persistenceMode==='indexeddb')check=await idbGet();else{const raw=localStorage.getItem(STORAGE_KEY);check=raw?JSON.parse(raw):null}verified=!!check&&JSON.stringify(backupSummary(check))===JSON.stringify(backupSummary(state))}catch(e){verified=false}}if(!verified){state=previous;await persistState();btn.disabled=false;btn.textContent='Backup einspielen';toast('Backup konnte nicht sicher gespeichert werden. Aktuelle Daten wurden beibehalten.','bad');return}if(syncStatus.enabled)window.VTFamilySync?.markAllLocalDocumentsDirty?.();closeModal();renderAll();toast(syncStatus.enabled?'Backup geprüft und eingespielt. Familiensync übernimmt den neuen Stand.':'Backup vollständig geprüft und eingespielt.','good')};
  }catch(e){console.warn(e);toast(e?.message||'Backup ist ungültig oder konnte nicht gelesen werden.','bad')}
}
async function resetAppData(){
  const syncStatus=window.VTFamilySync?.status?.();
  if(syncStatus?.enabled){
    modal('<div class="eyebrow">Gefahrenbereich</div><h2>Gesamtlöschung im Familiensync gesperrt</h2><p>Ein vollständiger Reset würde Profile auf diesem Gerät entfernen, ohne deren Löschung mit der aktuellen Sync-Version eindeutig an alle Familiengeräte übertragen zu können.</p><div class="notice warn"><strong>Es wird nichts gelöscht.</strong><br>Einzelne Lernstände können über „Lernstoff löschen“ sicher synchronisiert geleert werden. Für einen rein lokalen Neustart zuerst die Familienverbindung bewusst trennen.</div><div class="modal-actions wrap"><button type="button" id="backupBeforeBlockedReset" class="secondary">Backup erstellen</button><button value="cancel" class="primary">Schließen</button></div>');
    $('#backupBeforeBlockedReset').onclick=backup;return;
  }
  modal(`<div class="eyebrow">Gefahrenbereich</div><h2>Alle persönlichen App-Daten löschen</h2><p>Profile, Lernbereiche, Testdaten, Lernfortschritt und Noten werden gelöscht. Die fest mit der App ausgelieferte, geprüfte Lehrwerksbibliothek wird anschließend automatisch wiederhergestellt.</p><label>Zur Bestätigung <strong>LÖSCHEN</strong> eingeben<input id="resetConfirm" autocomplete="off"></label><div class="modal-actions"><button value="cancel" class="ghost">Abbrechen</button><button type="button" id="confirmReset" class="danger-btn" disabled>Persönliche Daten löschen</button></div>`);
  const input=$('#resetConfirm'),btn=$('#confirmReset');input.oninput=()=>btn.disabled=input.value.trim().toUpperCase()!=='LÖSCHEN';
  btn.onclick=async()=>{await persistChain.catch(()=>{});try{if(persistenceMode==='indexeddb')await idbClear();else localStorage.removeItem(STORAGE_KEY)}catch(e){console.warn(e)}state=defaultState();await installBuiltinLibraries();await persistState();closeModal();showView('homeView');renderAll();toast('Persönliche App-Daten gelöscht; geprüfte Lehrwerksvokabeln bleiben erhalten.','good')};
}
