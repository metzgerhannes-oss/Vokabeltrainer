...
async function resetAppData(){
  ...
  btn.onclick=async()=>{await persistChain.catch(()=>{});try{if(persistenceMode==='indexeddb')await idbClear();else localStorage.removeItem(STORAGE_KEY)}catch(e){console.warn(e)}state=defaultState();await installBuiltinLibraries();await persistState();try{await window.VTFamilySync?.replaceCloudWithCurrent?.()}catch(e){console.warn('Bereinigter Familienstand konnte noch nicht synchronisiert werden.',e)}closeModal();showView('homeView');renderAll();toast('Persönliche App-Daten gelöscht; geprüfte Lehrwerksvokabeln bleiben erhalten.','good')};
}
