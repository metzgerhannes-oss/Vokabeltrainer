...
function gradeLeitnerCard(w,answer,q){
  ...
  $('#studyArea .study-card').insertAdjacentHTML('beforeend',`<div class="feedback notice ${ok?'good':'bad'}"><strong>${ok?'Richtig.':'Noch nicht richtig.'}</strong><br>${ok?'':errorFeedbackHtml(answer,q.targets)}<div class="answer-pronunciation"><span><strong>${esc(w.term)}</strong> · ${esc(w.translation)}</span>${audioButtonHtml(w.term,'Anhören')}</div><div class="leitner-move ${ok?'forward':'back'}">${esc(movement)}</div>${mastered?'<div class="leitner-mastered">✓ Nachhaltig gemeistert</div>':''}</div>`);
...
function wordLearningCard(w,compact=false){const chunks=learningChunksFor(w);return `<div class="learning-card"><div><small>Wort</small><div class="learning-audio-row"><strong>${esc(w.term)}</strong>${audioButtonHtml(w.term,'Anhören')}</div>${w.extra?` · ${esc(w.extra)}`:''}</div><div><small>Bedeutung</small>${esc(w.translation)}</div>${w.example?`<div><small>Kontext</small><div class="learning-audio-row"><span>${esc(w.example)}</span>${audioButtonHtml(w.example,'Satz anhören')}</div></div>`:''}${w.mnemonic?`<div><small>Wortkniff / Eselsbrücke</small>${esc(w.mnemonic)}</div>`:''}${!compact&&chunks.length>1?`<div><small>Lernbausteine</small>${esc(chunks.join(' · '))}</div>`:''}</div>`}
function renderChunks(w){
  ...
  $('#studyArea').innerHTML=`<div class="study-card"><div class="eyebrow">Rechtschreibung · Wortbausteine</div><button id="speakBtn" class="secondary">🔊 Anhören</button><div class="study-prompt compact-prompt">${esc(q.prompt)}</div><div class="study-sub">Baue das Wort aus sinnvollen Lernbausteinen. Bei ganzen Sätzen wird diese Übung nicht verwendet.</div><div id="assembled" class="assembled">&nbsp;</div><div class="word-chunks">${shuffled.map(c=>`<button class="chunk" data-chunk="${esc(c.x)}">${esc(c.x)}</button>`).join('')}</div><div class="row gap center-actions"><button id="chunkReset" class="ghost">Neu</button><button id="chunkCheck" class="primary">Prüfen</button></div>${cardExtras(w)}</div>`;
...
function audioButtonHtml(text,label='Anhören'){const value=String(text||'').trim();if(!value)return '';return `<button type="button" class="ghost audio-inline" data-speak="${esc(value)}" aria-label="${esc(label)}">🔊<span>${esc(label)}</span></button>`}
...
function gradeChoice(btn,w,answer,target,skill,nonEvaluative=false,questionSnapshot=null){
  if(session.locked)return;const targetSession=session;session.locked=true;const q=questionSnapshot||currentQuizQuestion(w,session.currentSubmode||skill),grade=gradeQuizQuestion(q,answer),ok=grade.correct,before=leitnerBox(w);
  btn.classList.add(ok?'correct':'wrong');if(!ok){Array.from(document.querySelectorAll('[data-answer]')).find(b=>gradeQuizQuestion(q,b.dataset.answer).correct)?.classList.add('correct')}
  if(['recognition','listening'].includes(skill))session.scaffoldedWords[w.id]=true;
  if(nonEvaluative){recordNonEvaluative(w,'flash',ok,skill)}
  else{
    recordResult(w,ok,skill,ok?null:skill);
    logSessionResult(w,{answer,target:q.targets,correct:ok,skill,orthographyOk:grade.orthographyOk,prompt:q.prompt,boxBefore:before,boxAfter:leitnerBox(w),reason:sessionResultReason({correct:ok,orthographyOk:grade.orthographyOk,answer,targets:q.targets,mode:skill,assisted:!!session.hintUsed})});
    if(!ok){$('#studyArea .study-card')?.insertAdjacentHTML('beforeend',`<div class="feedback notice bad">${wordLearningCard(w,true)}</div>`);maybeSpeakCorrection(w)}
  }
  scheduleSessionAdvance(targetSession,ok,w,650);
}
function gradeText(w,answer,target,errorType,skill){
  if(session.locked)return;const targetSession=session;session.locked=true;const q=currentQuizQuestion(w,session.currentSubmode||skill),grade=gradeQuizQuestion(q,answer),ok=grade.correct,before=leitnerBox(w);
  const detail=ok?(session.hintUsed?'Richtig mit Hinweis.':'Richtig.'):' ';
  const pronunciation=`<div class="answer-pronunciation"><span>Vokabel: <strong>${esc(w.term)}</strong></span>${audioButtonHtml(w.term,'Anhören')}</div>`;
  $('#studyArea .study-card').insertAdjacentHTML('beforeend',`<div class="feedback notice ${ok?'good':'bad'}">${ok?`<strong>${detail}</strong>`:errorFeedbackHtml(answer,q.targets)}${pronunciation}${!ok?wordLearningCard(w):''}</div>`);
  recordResult(w,ok,skill,ok?null:errorType,{orthographyOk:grade.orthographyOk});
  logSessionResult(w,{answer,target:q.targets,correct:ok,skill,orthographyOk:grade.orthographyOk,assisted:!!session.hintUsed,prompt:q.prompt,boxBefore:before,boxAfter:leitnerBox(w),reason:sessionResultReason({correct:ok,orthographyOk:grade.orthographyOk,answer,targets:q.targets,mode:skill,assisted:!!session.hintUsed})});
  if(!ok)maybeSpeakCorrection(w);
  scheduleSessionAdvance(targetSession,ok,w,ok?700:2400);
}
...
function recordResult(w,ok,skill,errorType,opts={}){
  ...
  const assisted=!!(ok&&session.hintUsed),supportMode=['recognition','listening','chunks'].includes(session.currentSubmode),active=isActiveSkill(skill)&&!supportMode,now=new Date().toISOString(),firstActiveToday=active&&!(w.activePracticeDays||[]).includes(today()),cold=active&&!assisted&&skill!=='spelling'&&firstActiveToday&&!session.scaffoldedWords?.[w.id];
...
function sessionResultsText(results=session?.results||[]){
  ...
  for(const r of results){
    ...
    lines.push('',r.order+'. '+r.mode+' · '+status,(r.setTitle?'Lernset: '+r.setTitle:''),'Frage: '+(r.prompt||'–'),'Meine Antwort: '+(r.answer||'–'),'Erwartet: '+((r.targets||[]).join(' | ')||'–'),r.reason?'Warum: '+r.reason:'',boxMove,r.assisted?'Mit Hilfe: ja':'Mit Hilfe: nein');
  }
...
function sessionResultsHtml(results=[]){
  ...
  return `<section class="session-review" aria-labelledby="sessionReviewTitle"><div class="row spread align-center wrap"><div><div class="eyebrow">Ergebnisübersicht</div><h3 id="sessionReviewTitle">Alle Abfragen dieser Einheit</h3><p class="session-review-intro">Du siehst genau, was gefragt, geantwortet und bewertet wurde.</p></div><button id="copySessionResultsBtn" class="ghost">Ergebnisse kopieren</button></div><div class="session-result-list">${results.map(r=>{const cls=!r.correct?'bad':r.orthographyOk===false?'warn':'good',status=!r.correct?'Falsch':r.orthographyOk===false?'Richtig erinnert · Schreibweise':'Richtig',before=Number(r.boxBefore),after=Number(r.boxAfter),hasBox=before>=1&&before<=5&&after>=1&&after<=5,box=hasBox?`<span class="session-box-move" aria-label="Karteikasten Box ${before} zu Box ${after}">Box ${before} → Box ${after}</span>`:'',audio=r.term?audioButtonHtml(r.term,'Anhören'):'';return `<article class="session-result ${cls}" data-session-result="${r.correct&&r.orthographyOk!==false?'correct':'review'}" data-word-id="${esc(r.wordId||'')}"><div class="session-result-head"><strong>${r.order}. ${esc(r.mode)}</strong><div class="session-result-status"><span class="pill">${esc(status)}</span>${box}</div></div>${r.setTitle?`<small class="session-result-set">${esc(r.setTitle)}</small>`:''}<div class="session-result-grid"><div><small>Frage</small><strong>${esc(r.prompt||'–')}</strong></div><div><small>Deine Antwort</small><span>${esc(r.answer||'–')}</span></div><div><small>Richtige / akzeptierte Antwort</small><span>${esc((r.targets||[]).join(' · ')||'–')}</span>${audio}</div></div>${r.reason?`<div class="session-result-reason"><small>Bewertung</small><span>${esc(r.reason)}</span></div>`:''}${r.note?`<small class="session-result-note">${esc(r.note)}</small>`:r.assisted?'<small class="session-result-note">Mit Hilfe beantwortet</small>':''}</article>`}).join('')}</div></section>`;
}
