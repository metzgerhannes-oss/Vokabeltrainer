...
function renderRecall(w){
  $('#studyArea').innerHTML=`<div class="study-card"><div class="eyebrow">Aktiver Abruf</div><div class="study-prompt">${esc(w.translation)}</div><div class="study-sub">Schreibe die Vokabel aus dem Gedächtnis.</div><input id="answerField" class="answer-input" aria-label="Deine Antwort" autocomplete="off" autocapitalize="none"><div class="row gap center-actions top-space"><button id="answerBtn" class="primary">Prüfen</button><button id="hintBtn" class="ghost">Hinweis</button></div>${cardExtras(w)}</div>`;
