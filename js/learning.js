...
function renderFirstContactFinish(){
  const setId=session?.setId,set=state.sets.find(x=>x.id===setId),status=firstContactStatus(setId),daily=!!session?.isDaily,batchTotal=session?.queue?.length||0,battleEarned=grantBattleTicket(daily?'dailyFirstContact':'firstContact');
