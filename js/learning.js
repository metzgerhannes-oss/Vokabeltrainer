...
function renderPracticeTest(w){
  ...
  const submit=()=>{if(session.locked)return;session.locked=true;const answer=$('#answerField').value.trim();const ok=practiceAnswerMatches(answer,d);session.testAnswers.push({wordId:w.id,answer,correct:ok,target:d.target,acceptedTargets:d.targets||[d.target],prompt:d.prompt,strictOrthography:!!d.strictOrthography});session.answered++;if(ok)session.correct++;session.index++;renderStudy()};
...
function renderRecognition(w){
  ...
  $$('[data-answer]').forEach(b=>b.onclick=()=>gradeChoice(b,w,b.dataset.answer,q.targets,'recognition'));
}
