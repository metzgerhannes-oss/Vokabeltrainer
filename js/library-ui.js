...
renderLibrary=function(){
  ...
  const desktopRows=visible.map(v=>'<tr><td><div class="learning-audio-row"><strong>'+esc(v.term)+'</strong><button type="button" class="ghost audio-inline" data-speak="'+esc(v.term)+'" aria-label="Vokabel anhören">🔊</button></div>'+(v.extra?'<br><small>'+esc(v.extra)+'</small>':'')+(v.termVariants?.length?'<br><small>Varianten: '+esc(v.termVariants.join(' · '))+'</small>':'')+'</td><td>'+meanings(v)+'</td><td>'+esc(info(v))+'</td><td><button class="ghost" data-vocab-edit="'+esc(v.id)+'">Bearbeiten</button></td></tr>').join('');
