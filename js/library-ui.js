...
renderLibrary=function(){
  ...
  if($('#libraryBookFilter'))$('#libraryBookFilter').innerHTML='<option value="">Alle Lehrwerke</option>'+books.map(b=>'<option value="'+esc(b.id)+'" '+(bookFilter===b.id?'selected':'')+'>'+esc(b.title||formatIsbn(b.isbn13))+' · '+esc(formatIsbn(b.isbn13))+'</option>').join('');
  ...
  if($('#libraryCountPill'))$('#libraryCountPill').textContent=allWords.length+' globale'+(allWords.length===1?' Vokabel':' Vokabeln')+(books.length?' · '+books.length+' Lehrwerk'+(books.length===1?'':'e'):'');
  const visible=filtered.slice(0,libraryRenderLimit),more=filtered.length-visible.length;
  if(!allWords.length){$('#wordLibrary').innerHTML='<div class="empty-state library-empty"><strong>Noch keine Vokabeln</strong><p>Ein importiertes Wort wird hier einmal global gespeichert und kann danach von allen Profilen wiederverwendet werden.</p><button id="emptyLibraryAddBtn" class="primary">+ Vokabeln hinzufügen</button></div>';$('#emptyLibraryAddBtn')?.addEventListener('click',openLibraryAddMenu);return}
  ...
  const desktopRows=visible.map(v=>'<tr><td><strong>'+esc(v.term)+'</strong>'+(v.extra?'<br><small>'+esc(v.extra)+'</small>':'')+(v.termVariants?.length?'<br><small>Varianten: '+esc(v.termVariants.join(' · '))+'</small>':'')+'</td><td>'+meanings(v)+'</td><td>'+esc(info(v))+'</td><td><button class="ghost" data-vocab-edit="'+esc(v.id)+'">Bearbeiten</button></td></tr>').join('');
  const mobileRows=visible.map(v=>'<button class="library-word-card" data-vocab-edit="'+esc(v.id)+'"><span class="library-word-main"><strong>'+esc(v.term)+'</strong><span>'+mobileMeaning(v)+'</span></span><span class="library-word-meta">'+esc(info(v))+'</span><span class="library-word-chevron">›</span></button>').join('');
  $('#wordLibrary').innerHTML='<div class="library-result-line"><span>'+(filtered.length===allWords.length?allWords.length+' globale Vokabel'+(allWords.length===1?'':'n'):filtered.length+' von '+allWords.length)+'</span><span class="microcopy">Suche über Wort · Bedeutung · ISBN · Unit</span></div><div class="library-table-desktop table-wrap"><table><thead><tr><th>Vokabel</th><th>Bedeutungen</th><th>Verwendung</th><th></th></tr></thead><tbody>'+desktopRows+'</tbody></table></div><div class="library-word-list">'+mobileRows+'</div>'+(more?'<div class="load-more"><button id="libraryMoreBtn" class="ghost">Weitere '+Math.min(200,more)+' anzeigen</button><span class="microcopy">'+visible.length+' von '+filtered.length+' sichtbar</span></div>':'');
