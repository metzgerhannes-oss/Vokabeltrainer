...
function pruneState(){
  ...
  state.learners?.forEach(l=>{if(l.campaignLog?.length>500)l.campaignLog=l.campaignLog.slice(-500);if(l.streakDays?.length>900)l.streakDays=l.streakDays.slice(-900);if(l.testFortresses&&typeof l.testFortresses==='object'){const entries=Object.entries(l.testFortresses).sort((a,b)=>String(a[1]?.testDate||'').localeCompare(String(b[1]?.testDate||'')));if(entries.length>80)l.testFortresses=Object.fromEntries(entries.slice(-80));}});
...
function mergeProgress(target,source){
  ...
  const sourceLeitnerNewer=String(source.leitnerUpdatedAt||'')>String(target.leitnerUpdatedAt||'');if(sourceLeitnerNewer){target.leitnerBox=clamp(Math.round(Number(source.leitnerBox)||1),1,5);target.leitnerUpdatedAt=source.leitnerUpdatedAt||null}
...
function migrateSenseModel(s){
  ...
  for(const row of (s.bookVocabulary||[])){
    const v=vocabById.get(row.vocabId);if(!v)continue;let sense=senseById(v,row.senseId);if(!sense&&row.translationOverride)sense=senseMatch(v,row.translationOverride);
    if(!sense){
      if((v.senses||[]).length>1){row.verifiedAt='';row.verificationBlocked=true;}
      sense=primarySense(v);
    }
...
function backfillFirstContact(s){
  if(Number(s?.firstContactVersion)>=1)return s;
  const setById=new Map((s?.sets||[]).map(set=>[String(set?.id||''),set])),now=new Date().toISOString();
  for(const link of (s?.setVocabulary||[])){
    const set=setById.get(String(link?.setId||''));if(!set||set.pairReviewRequired===true)continue;
    const at=String(link.firstContactCompletedAt||link.createdAt||now);
    link.firstContactCopiedAt=String(link.firstContactCopiedAt||at);
    link.firstContactRecalledAt=String(link.firstContactRecalledAt||at);
    link.firstContactCompletedAt=String(link.firstContactCompletedAt||at);
  }
  s.firstContactVersion=1;
  return s;
}
...
function backfillBookVocabularyVerification(s){
  ...
  for(const row of rows){
    if(!row||row.verifiedAt||row.verificationBlocked)continue;
...
function hardenState(s){
  ...
  s.learners=learnersIn.map((raw,i)=>{const l=raw&&typeof raw==='object'?raw:{};const old=String(l.id||'');const id=safeId(old,'learner',learnerUsed);if(!learnerMap.has(old))learnerMap.set(old,id);const battleTickets={...defaultSubjectNumbers(),...(l.battleTickets||{})};for(const subject of knownSubjectIds())battleTickets[subject]=Math.round(safeNumber(battleTickets[subject],0,3,0));return {...l,id,name:safeText(l.name||`Profil ${i+1}`,80),gradeLevel:/^(?:[1-9]|1[0-3])$/.test(String(l.gradeLevel||''))?String(l.gradeLevel):'',activeSubjects:normalizeLearnerSubjects(l,subjectHints.get(old)||[]),lrsMode:!!l.lrsMode,xp:Math.round(safeNumber(l.xp,0,100000000,0)),fontSize:safeNumber(l.fontSize,16,24,17),letterSpacing:safeNumber(l.letterSpacing,0,3,0),flashSpeed:Math.round(safeNumber(l.flashSpeed,800,4000,1600)),autoSpeakCorrection:l.autoSpeakCorrection!==false,battleTickets}});
  ...
  const linkUsed=new Set(),linkMap=new Map(),seenLink=new Set(),setPositions=new Map();s.setVocabulary=[];
  for(const raw of rawSetVocabulary){
    const x=raw&&typeof raw==='object'?raw:{},setId=setMap.get(String(x.setId||''))||String(x.setId||''),vocabId=vocabMap.get(String(x.vocabId||''))||String(x.vocabId||'');if(!setIds.has(setId)||!vocabIds.has(vocabId))continue;const set=s.sets.find(z=>z.id===setId),v=s.vocabulary.find(z=>z.id===vocabId);if(!set||!v||set.subject!==v.subject)continue;let senseId=senseMap.get(String(x.senseId||''))||String(x.senseId||''),sense=senseById(v,senseId);if(!sense&&x.translationOverride)sense=senseMatch(v,x.translationOverride);if(!sense)sense=primarySense(v);if(!sense)continue;senseId=sense.id;const combo=`${setId}\u0000${senseId}`;if(seenLink.has(combo))continue;seenLink.add(combo);const oldLinkId=String(x.id||''),id=safeId(x.id,'sv',linkUsed);if(oldLinkId&&!linkMap.has(oldLinkId))linkMap.set(oldLinkId,id);const next=(setPositions.get(setId)||0)+1,position=Math.max(next,Math.round(safeNumber(x.position,1,100000,next)));setPositions.set(setId,position);s.setVocabulary.push(makeSetVocabulary(setId,vocabId,senseId,{...x,id,position,termOverride:safeText(x.termOverride,300),translationOverride:safeText(x.translationOverride,700),acceptedTermOverrides:(Array.isArray(x.acceptedTermOverrides)?x.acceptedTermOverrides:[]).slice(0,20).map(y=>safeText(y,300)).filter(Boolean),acceptedTranslationOverrides:(Array.isArray(x.acceptedTranslationOverrides)?x.acceptedTranslationOverrides:[]).slice(0,20).map(y=>safeText(y,700)).filter(Boolean),extraOverride:safeText(x.extraOverride,700),exampleOverride:safeText(x.exampleOverride,2000),source:safeText(x.source,80),firstContactCopiedAt:safeText(x.firstContactCopiedAt||'',40),firstContactRecalledAt:safeText(x.firstContactRecalledAt||'',40),firstContactCompletedAt:safeText(x.firstContactCompletedAt||'',40),firstContactProvedAt:safeText(x.firstContactProvedAt||'',40)}));
  ...
  const validLinkIds=new Set(s.setVocabulary.map(x=>x.id));
  s.sets.forEach(set=>{set.testSelectedLinkIds=(set.testSelectedLinkIds||[]).map(id=>linkMap.get(String(id))||String(id)).filter(id=>validLinkIds.has(id));});
  ...
  s.learners.forEach(l=>{const hints=(s.sets||[]).filter(x=>x.learnerId===l.id).map(x=>normalizeSubjectId(x.subject));if(l.id===s.activeLearnerId)hints.push(normalizeSubjectId(s.activeSubject));l.gradeLevel=/^(?:[1-9]|1[0-3])$/.test(String(l.gradeLevel||''))?String(l.gradeLevel):'';l.activeSubjects=normalizeLearnerSubjects(l,hints);l.streakDays=l.streakDays||[];l.milestones=l.milestones||{};l.fortressWins={...defaultSubjectArrays(),...(l.fortressWins||{})};l.fortressWinsByYear=l.fortressWinsByYear||{};const safeBattleDays={};for(const [battleKey,rawDay] of Object.entries(l.battleDays&&typeof l.battleDays==='object'&&!Array.isArray(l.battleDays)?l.battleDays:{}).slice(-60)){const key=safeText(battleKey,80),day=rawDay&&typeof rawDay==='object'?rawDay:{};if(!/^\d{4}-\d{2}-\d{2}:[a-z0-9_-]+$/i.test(key))continue;safeBattleDays[key]={date:safeText(day.date||key.slice(0,10),10),subject:normalizeSubjectId(day.subject||key.slice(11)),unlocked:day.unlocked===true,rewardClaimed:day.rewardClaimed===true,attempts:Math.round(safeNumber(day.attempts,0,10000,0)),wins:Math.round(safeNumber(day.wins,0,10000,0)),unlockedAt:safeText(day.unlockedAt||'',40),rewardClaimedAt:safeText(day.rewardClaimedAt||'',40),lastAttemptAt:safeText(day.lastAttemptAt||'',40),lastResult:['win','hold','damage','secure'].includes(day.lastResult)?day.lastResult:'',lastRewarded:day.lastRewarded===true,actionUsed:day.actionUsed===true||(Number(day.attempts)||0)>0,actionUsedAt:safeText(day.actionUsedAt||'',40),fortressKey:safeText(day.fortressKey||'',180)};}l.battleDays=safeBattleDays;l.testFortresses=l.testFortresses&&typeof l.testFortresses==='object'&&!Array.isArray(l.testFortresses)?l.testFortresses:{};l.campaignLog=l.campaignLog||[];l.dailyPlans=l.dailyPlans||{};l.testSeries={...defaultTestSeries(),...(l.testSeries||{})};l.gradeScales=l.gradeScales||defaultGradeScales();knownSubjectIds().forEach(subject=>{l.gradeScales[subject]={...defaultGradeScale(),...(l.gradeScales[subject]||{})};const cfg=l.testSeries[subject];if(cfg?.setId){const mapped=setMap.get(String(cfg.setId))||String(cfg.setId);if(setIds.has(mapped)){cfg.setId=mapped;cfg.scopeMode=['range','selected'].includes(cfg.scopeMode)?cfg.scopeMode:'set';cfg.selectedLinkIds=(Array.isArray(cfg.selectedLinkIds)?cfg.selectedLinkIds:[]).map(id=>linkMap.get(String(id))||String(id)).filter(id=>validLinkIds.has(id));}else l.testSeries[subject]=null;}const key=`${subject}:${currentSchoolYear()}`;if(!l.fortressWinsByYear[key]&&Array.isArray(l.fortressWins?.[subject])&&l.fortressWins[subject].length)l.fortressWinsByYear[key]=[...l.fortressWins[subject]]});for(const plan of Object.values(l.dailyPlans)){if(Array.isArray(plan.wordIds))plan.wordIds=[...new Set(plan.wordIds.map(remapProgress).filter(id=>progressIds.has(id)))];if(Array.isArray(plan.wordRefs))plan.wordRefs=plan.wordRefs.map(r=>({...r,wordId:remapProgress(r.wordId)})).filter(r=>progressIds.has(r.wordId));}});
