...
function renderToday(){
  ...
  const cards=$('#practiceCardsBtn'),weakBtn=$('#practiceWeakBtn'),allBtn=$('#practiceAllBtn'),special=$('#practiceSpecialBtn');
  if(cards)cards.disabled=!cardPool.length;if(allBtn)allBtn.disabled=!cardPool.length;if(weakBtn)weakBtn.disabled=!weak.length;if(special)special.disabled=!cardPool.length;
  const recs=[
    {icon:'🔊',title:'Hören',sub:'Aussprache gezielt trainieren',mode:'listening'},
    ...
    {icon:'✍',title:'Handschrift',sub:'nachfahren · abdecken · selbst prüfen',mode:'handwriting'},
    {icon:'📝',title:'Abschreiben',sub:'freiwillige zusätzliche Schreibeinheit',mode:'copy'},
...
function openSetPairAudit(setId){
  ...
  const rows=words.map((w,i)=>`<tr><td>${i+1}</td><td><div class="learning-audio-row"><strong>${esc(w.term)}</strong><button type="button" class="ghost audio-inline" data-speak="${esc(w.term)}" aria-label="Vokabel anhören">🔊<span>Anhören</span></button></div></td><td>${esc(w.translation)}</td><td><small>${esc(termTargets(w).join(' · '))}</small></td><td><small>${esc(translationTargets(w).join(' · '))}</small></td></tr>`).join('');
...
function renderLibrary(){
  ...
  const desktopRows=visible.map(v=>`<tr><td><div class="learning-audio-row"><strong>${esc(v.term)}</strong><button type="button" class="ghost audio-inline" data-speak="${esc(v.term)}" aria-label="Vokabel anhören">🔊</button></div>${v.extra?`<br><small>${esc(v.extra)}</small>`:''}${v.termVariants?.length?`<br><small>Varianten: ${esc(v.termVariants.join(' · '))}</small>`:''}</td><td>${meanings(v)}</td><td>${esc(info(v))}</td><td><button class="ghost" data-vocab-edit="${v.id}">Bearbeiten</button></td></tr>`).join('');
