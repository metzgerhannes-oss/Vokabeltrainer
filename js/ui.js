...
function renderToday(){
  ...
  if(!hasWords){
    ...
    $('#todayContext').textContent=parent?'Lege den Lernstoff fest oder ergänze die Bibliothek.':'Bitte einen Erwachsenen, neue Vokabeln vorzubereiten.';
...
function renderSets(){
  const sets=mySets(); if(!sets.length){$('#setList').innerHTML='<div class="empty-state"><strong>Noch kein Lernstoff vorbereitet</strong><p>Wähle ein Kind, ein Lehrwerk und die Vokabeln. Die technischen Lernsets legt die App im Hintergrund an.</p><button id="emptyNewSetBtn" class="primary">Lernstoff festlegen</button></div>';$('#emptyNewSetBtn').onclick=()=>openLearningContentPlanner();return}
...
function renderLibrary(){
  ...
  if(!words.length){$('#wordLibrary').innerHTML='<div class="empty-state library-empty"><strong>Noch keine Vokabeln</strong><p>Neue Wörter werden über „Lernstoff festlegen“ erfasst oder aus einem Lehrwerk übernommen.</p><button id="emptyLibraryAddBtn" class="primary">Lernstoff festlegen</button></div>';$('#emptyLibraryAddBtn')?.addEventListener('click',openLearningContentPlanner);return;}
...
function renderParentOverview(){
  ...
  if(!mySets().length)tasks.push('<div class="parent-task"><div><strong>Lernstoff festlegen</strong><small>Noch keine Vokabeln für dieses Kind vorbereitet.</small></div><button class="primary" data-parent-newset>Lernstoff auswählen</button></div>');
