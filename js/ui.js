...
function renderFamilySync(){
  ...
  box.className='notice good';box.innerHTML=`<strong>Familiensync aktiv</strong><br>Familiencode: ${esc(s.familyCode||s.familyId)} · ${s.role==='parent'?'Eltern-Gerät':'Kindergerät'} · zuletzt ${esc(familySyncTime(s.lastSync))}${s.dirty?` · ${s.dirty} Änderung${s.dirty===1?'':'en'} wartet${s.dirty===1?'':'en'} auf Upload`:''}`;
}
function openFamilySyncSetup(){
  if(!window.VTFamilySync)return;
  if(VTFamilySync.status().enabled){openFamilySyncSwitch();return}
  modal('<div class="eyebrow">Familie & Geräte</div><h2>Familiensync einrichten</h2><p>Nutze auf allen Geräten denselben Familienverbund.</p><div class="notice subtle"><strong>Kindergerät verbinden?</strong><br>Auf dem bereits verbundenen Eltern-Gerät „Kindergerät hinzufügen“ wählen und den Link auf dem Kindergerät öffnen. Dort müssen weder Familien-ID noch PIN eingegeben werden.</div><div class="notice subtle"><strong>Weiteres Eltern-Gerät?</strong><br>Dann der bestehenden Familie beitreten – keine neue Familie anlegen.</div><div class="modal-actions stack-mobile"><button type="button" id="familySyncJoinChoiceBtn" class="primary">Bestehender Familie beitreten</button><button type="button" id="familySyncCreateChoiceBtn" class="secondary">Neue Familie anlegen</button><button value="cancel" class="ghost">Abbrechen</button></div>');
...
function openFamilySyncJoin(prefillId=''){
  const normalized=window.VTFamilySync?.normalizeFamilyId?.(prefillId)||'';
  const shown=window.VTFamilySync?.formatFamilyId?.(normalized)||'';
  modal(`<div class="eyebrow">Familie & Geräte</div><h2>Bestehender Familie beitreten</h2><p>Für ein weiteres Eltern-Gerät genügt der Familiencode vom bereits verbundenen Eltern-Gerät und dieselbe Familien-PIN.</p><label>Familiencode<input id="familyJoinId" type="text" autocomplete="off" spellcheck="false" inputmode="text" placeholder="8DEC-1826-8742" value="${esc(shown)}"></label><label>Familien-PIN<input id="familyJoinPin" type="password" minlength="6" autocomplete="current-password" placeholder="mindestens 6 Zeichen"></label><div id="familySyncJoinError" class="notice subtle">Du kannst den Code mit oder ohne Bindestriche eingeben. Kindergeräte werden stattdessen über „Kindergerät hinzufügen“ per Link verbunden.</div><div class="modal-actions"><button type="button" id="familySyncBackBtn" class="ghost">Zurück</button><button type="button" id="familySyncJoinBtn" class="primary">Familie beitreten</button></div>`);
  $('#familySyncBackBtn').onclick=()=>VTFamilySync.status().enabled?openFamilySyncSwitch():openFamilySyncSetup();
  $('#familySyncJoinBtn').onclick=async()=>{const id=VTFamilySync.normalizeFamilyId($('#familyJoinId').value),pin=$('#familyJoinPin').value,err=$('#familySyncJoinError'),btn=$('#familySyncJoinBtn');if(!id){err.className='notice warn';err.textContent='Bitte einen gültigen Familiencode eingeben, z. B. 8DEC-1826-8742.';return}if(pin.length<6){err.className='notice warn';err.textContent='Die Familien-PIN muss mindestens 6 Zeichen lang sein.';return}btn.disabled=true;btn.textContent='Wird verbunden …';try{await VTFamilySync.joinParent(id,pin,'Eltern-Gerät');closeModal();renderAll();VTFamilySync.bootstrap();toast('Dieses Gerät ist jetzt mit der bestehenden Familie verbunden.','good')}catch(e){err.className='notice bad';err.textContent=e.message||'Beitritt fehlgeschlagen.';btn.disabled=false;btn.textContent='Familie beitreten'}};
}
...
function openChildDeviceInviteFallback(){
...
function bind(){
  ...
  $('#familySyncSetupBtn').onclick=openFamilySyncSetup; $('#familySyncNowBtn').onclick=runFamilySync; $('#familySyncChildBtn').onclick=()=>{const fn=window.openChildDeviceInvite||openChildDeviceInviteFallback;fn()}; $('#familySyncSwitchBtn').onclick=openFamilySyncSwitch;
