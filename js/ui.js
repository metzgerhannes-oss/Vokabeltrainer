...
function openTestDatePlanner(){
  ...
  const prepareCaptureSet=source=>{
    ...
    const set={id:uid('set'),learnerId:l.id,subject,title,schoolYear:currentSchoolYear(),bookId:book?.id||'',bookSection:'',testDate:when,testScopeMode:'set',testSelectedLinkIds:[],testFrom:1,testTo:0,testFormat:format.value,from:'',to:'',captureSource:source};
    state.sets.push(set);l.dailyPlans={};save();return set;
  };
...
function openProfileEditor(id=null){
  const existing=id?state.learners.find(x=>x.id===id):null,active=normalizeLearnerSubjects(existing||{},existing?[]:[availableSubjectIds()[0]||'english']),avatarStyle=existing?.avatarStyle==='female'?'female':'male';
